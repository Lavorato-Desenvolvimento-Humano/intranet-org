...
import com.intranet.drive.file.repository.FileSpecification;
...
public class FileService {
    ...
    @Transactional(readOnly = true)
    public Page<FileDto> searchAdvanced(String query, String type, LocalDateTime startDate,
                                        LocalDateTime endDate, Long minSize, Long maxSize, Pageable pageable) {
        UserDto currentUser = getCurrentUser();
        Long ownerIdFilter = null;

        // REGRA DE SEGURANÇA NA BUSCA:
        // Se o utilizador não for ADMIN, filtramos automaticamente para trazer apenas
        // os ficheiros que ele é dono (ownerId).
        // Isso garante que a paginação funcione corretamente vindo do banco.
        // Nota: Para incluir ficheiros partilhados com ele na busca, seria necessário um JOIN
        // mais complexo na Specification com a tabela de permissões.
        if (!coreIntegrationService.hasRole(currentUser, "ADMIN")) {
            ownerIdFilter = currentUser.getId();
        }

        Specification<FileEntity> spec = FileSpecification.withFilter(
                query,
                type,
                startDate,
                endDate,
        ...
        Page<FileEntity> files = fileRepository.findAll(spec, pageable);
    ...
    private void validateFileAccess(FileEntity file) {
        ...
        if(!drivePermissionService.hasReadAccess(file.getId(), currentUser)) {
            throw new SecurityException("Acesso negado ao arquivo: " + file.getName());
        }
