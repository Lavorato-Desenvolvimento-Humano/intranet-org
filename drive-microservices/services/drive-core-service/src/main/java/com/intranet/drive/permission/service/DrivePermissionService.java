package com.intranet.drive.permission.service;
...
import com.intranet.drive.permission.repository.FilePermissionRepository;
...
public class DrivePermissionService {
    ...
    private boolean checkRecursive(FileEntity file, UserDto user, PermissionType required) {
        ...
        for (FilePermissionEntity p : perms) {
            ...
            switch (p.getTargetType()) {
                case USER:
                    match = p.getTargetId().equals(String.valueOf(user.getId()));
                    break;
                case ROLE:
                    match = user.getRoles().contains(p.getTargetId());
                    break;
                case TEAM:
                    match = false;
                    break;
            ...
            if (match && isSufficient(p.getPermissionType(), required)) {
                return true;
            }
        }

        if (file.getFolderId() != null) {
            return fileRepository.findById(file.getFolderId())
                    .map(parent -> checkRecursive(parent, user, required))
                    .orElse(false);
        }
        return false;
    }
    ...
    public boolean hasReadAccess(Long fileId, UserDto user) {
        return hasPermission(fileId, user, PermissionType.READ);
    }

    public boolean hasWriteAccess(Long fileId, UserDto user) {
        return hasPermission(fileId, user, PermissionType.WRITE);
    }
